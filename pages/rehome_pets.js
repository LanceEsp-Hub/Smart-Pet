...
export default function RehomePets() {
  ...
  const FilterPanel = ({ showFilters, filters, onFilterChange, onClose }) => {
  const petTypes = ["Dog", "Cat", "Bird", "Rabbit", "Other"]
  const genders = ["Male", "Female"]
  const goodWithOptions = ["children", "dogs", "cats", "elderly", "strangers"]
  const energyLevels = ["Low", "Medium", "High"]

  return (
    <div className={`fixed inset-0 z-50 overflow-y-auto ${showFilters ? "block" : "hidden"}`}>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {goodWithOptions.map((option) => (
                    <button
                      key={option}
                      className={`px-3 py-1 rounded-full text-sm ${filters.good_with.includes(option) ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-800'}`}
                      onClick={() => {
                        const current = filters.good_with.split(',')
                        const updated = current.includes(option)
                          ? current.filter(item => item !== option)
                          : [...current, option].filter(Boolean)
                        onFilterChange({ good_with: updated.join(',') })
                      }}
                    >
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => onFilterChange({ energy_level: e.target.value })}
                >
...
const PetModal = ({ pet, showModal, onClose, canAdopt, onAdopt, activeTab, setActiveTab }) => {
  if (!pet) return null
    ...
    switch (activeTab) {
        ...
        return (
          <div className="space-y-4">
            <div>
              ...
              <p className="text-gray-600 mt-1">{pet.description || "No description provided."}</p>
            </div>
            <div>
            ...
            {pet.date && (
              <div>
                <h4 className="font-medium text-gray-900">Available Since</h4>
                <div className="flex items-center mt-1 text-gray-600">
        ...
        return (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <h4 className="font-medium text-gray-900">Vaccinated</h4>
                <p className="text-gray-600 mt-1">
                  {pet.health_info?.vaccinated ? "Yes" : "No"}
                </p>
              </div>
              <div>
                <h4 className="font-medium text-gray-900">Spayed/Neutered</h4>
                <p className="text-gray-600 mt-1">
                  {pet.health_info?.spayed_neutered ? "Yes" : "No"}
                </p>
              </div>
              <div>
                <h4 className="font-medium text-gray-900">Energy Level</h4>
                <p className="text-gray-600 mt-1">
                  {pet.health_info?.energy_level || "Not specified"}
                </p>
              </div>
            </div>
            <div>
              <h4 className="font-medium text-gray-900">Temperament</h4>
              <p className="text-gray-600 mt-1">
                {pet.health_info?.temperament_personality || "Not specified"}
              </p>
            </div>
            <div>
              <h4 className="font-medium text-gray-900">Reason for Adoption</h4>
              <p className="text-gray-600 mt-1">
                {pet.health_info?.reason_for_adoption || "Not specified"}
              </p>
            </div>
          </div>
        )
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-gray-600 mt-1">{pet.type}</p>
              </div>
              <div>
                <h4 className="font-medium text-gray-900">Gender</h4>
                <p className="text-gray-600 mt-1">{pet.gender}</p>
              </div>
            </div>
            <div>
              <h4 className="font-medium text-gray-900">Good With</h4>
              <div className="flex flex-wrap gap-2 mt-1">
                {pet.health_info?.good_with && Object.entries(pet.health_info.good_with).map(
                  ([key, value]) => value && (
      ...
      case "contact":
        return (
          <div className="space-y-4">
            <div>
              <h4 className="font-medium text-gray-900">Contact Owner</h4>
              <div className="mt-2 space-y-2">
                <p className="text-gray-600">
                  <span className="font-medium">Name:</span> {pet.owner_info?.name || "Not provided"}
                </p>
                <p className="text-gray-600">
                  <span className="font-medium">Phone:</span> {pet.owner_info?.phone || "Not provided"}
                </p>
              </div>
            </div>
          </div>
              ...
              <div className="mt-3 text-center sm:mt-0 sm:text-left w-full">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">{pet.type} • {pet.gender}</p>
                  </div>
                  <button onClick={onClose} className="text-gray-400 hover:text-gray-500">
                    <X size={24} />
                  </button>

...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    if (!userId) return;
    fetch(`http://localhost:8000/api/address/user/${userId}`)
      .then(res => res.json())
      .then(data => {
  ...
  useEffect(() => {
    if (!userId) return;
    fetch(`http://localhost:8000/api/ecommerce/cart?user_id=${userId}`)
      .then(res => res.json())
  ...
  useEffect(() => {
    ...
    const fetchUserVouchers = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:8000/api/vouchers/user/${userId}`);
        if (!response.ok) {
  ...
  useEffect(() => {
    const fetchDeliverySettings = async () => {
      try {
        const response = await fetch("http://localhost:8000/api/ecommerce/delivery-fee");
        if (!response.ok) {
  ...
  const validateVoucher = async () => {
    ...
    try {
      ...
      if (result.is_valid) {
        ...
        setVoucherCode("");
        alert(`Voucher applied! ${result.message}`);
      } else {
  ...
  const handlePlaceOrder = () => {
    ...
    if (deliveryType === "delivery" && (!selectedAddressId || addresses.length === 0)) {
      alert("Please select a shipping address for delivery orders.");
      return;
    }
    setShowModal(true);
  };

...
export default function CartPage() {
  ...
  useEffect(() => {
    const fetchCart = async () => {
      ...
      try {
        const res = await fetch(`http://localhost:8000/api/ecommerce/cart?user_id=${userId}`);
  ...
  const updateQuantity = async (cartItemId, newQuantity) => {
    try {
      const res = await fetch(`http://localhost:8000/api/ecommerce/cart/items/${cartItemId}`, {
        method: "PUT",

...
export default function UserProductPage() {
  ...
  useEffect(() => {
    const fetchProductsAndCart = async () => {
      try {
        ...
        // Fetch categories
        const categoriesRes = await fetch("http://localhost:8000/api/ecommerce/categories");
        if (categoriesRes.ok) {
        ...
        if (userId) {
          const cartRes = await fetch(`http://localhost:8000/api/ecommerce/cart?user_id=${userId}`);
          if (cartRes.ok) {
  ...
  const addToCart = async (productId, quantity) => {
    try {
      ...
      const response = await fetch(`http://localhost:8000/api/ecommerce/cart/items?user_id=${userId}`, {
        method: "POST",
        headers: {
